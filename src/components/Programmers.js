...
export default function Programmers() {
  ...
  const getNameOfFeatured = (programmer, featuredId) => {
    ...
    return setProgrammer(programmer[featuredId].name)
  ...
  return (
    <div className='widget-programmers container'>
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div key={dev.id}>
              {dev.name} <button onClick={() => setFeaturedId(dev.id) }>Feature</button> { /* set the featured id passing dev.id */ }
            </div>
          )
        }
      </div>
      {
        ...
        featuredId  
          ? <div style={style}>🎉 Let&apos;s celebrate {getNameOfFeatured(programmer, featuredId)}! 🥳</div>
